...
class exact_solver():
    ...
    def __init__(self, map: np.ndarray, iterations: int = 100, mu0: float = 0.5, rho: float = 1.1, alpha: float = 0.03,
                 gamma: float = 0.8, *args, **kwargs):
    ...
    def t_sub(self):
        ...
        d_v[1, 1] = -1
        d_v[0, 1] = 1'''
        d_h = gen_grad_kernel(x=1)
        d_v = gen_grad_kernel(x=0)
        F_Dh = np.multiply(np.fft.fftshift(np.fft.fft2(d_h, (self.m, self.n))).conj(), np.fft.fftshift(np.fft.fft2(d_h, (self.m, self.n))))
        F_Dv = np.multiply(np.fft.fftshift(np.fft.fft2(d_v, (self.m, self.n))).conj(), np.fft.fftshift(np.fft.fft2(d_v, (self.m, self.n))))
    ...
    def gamma_correction(self, epsilon: float = 1e-3):
        self.map_refined_gamma = np.clip(self.T, epsilon, 1) ** self.gamma
